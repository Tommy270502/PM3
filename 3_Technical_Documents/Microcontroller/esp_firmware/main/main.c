...
static const char remote_device_name[] = "ACTON III";
...
static void filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param)
{
    ...
    for (int i = 0; i < param->disc_res.num_prop; i++) {
        ...
        case ESP_BT_GAP_DEV_PROP_BDNAME: {
            size_t n = p->len < ESP_BT_GAP_MAX_BDNAME_LEN ? p->len : ESP_BT_GAP_MAX_BDNAME_LEN;
            memcpy(s_peer_bdname, p->val, n);
            s_peer_bdname[n] = '\0';
            break;
        }
        default:
    ...
    // (Optional) Keep the rendering-class filter, but you can remove it if it hides your speaker:
    if (!esp_bt_gap_is_valid_cod(cod) ||
        !(esp_bt_gap_get_cod_srvc(cod) & ESP_BT_COD_SRVC_RENDERING)) {
        // Comment out the 'return' below if the speaker isn't found due to strict COD
        // return;
    }

    // Prefer EIR name; fallback to BDNAME
    if (eir) get_name_from_eir(eir, s_peer_bdname, NULL);

    if (s_peer_bdname[0]) {
        ESP_LOGI(BT_AV_TAG, "--Name: %s  RSSI:%"PRId32, s_peer_bdname, rssi);

        // Exact or prefix match ("ACTON III", sometimes with suffix)
        if (strcmp((char *)s_peer_bdname, remote_device_name) == 0 ||
            strstr((char *)s_peer_bdname, remote_device_name) == (char *)s_peer_bdname) {

            ESP_LOGI(BT_AV_TAG, "Found target device: %s [%s]", s_peer_bdname, bda_str);
